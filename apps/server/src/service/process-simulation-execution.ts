...
import { simulationExecution, simulationExecutionLog, simulationExecutionTrade } from '@baron/db/schema';
...
import { getSimulationExecutionFrvpProfiles } from './simulation-execution/get-simulation-execution-frvp-profiles';
import { getSimulationInfoBars } from './simulation-execution/get-simulation-execution-info-bars';
import { getSimulationExecutionStartDate } from './simulation-execution/get-simulation-execution-start-date';
import { getSimulationExecutionToProcess } from './simulation-execution/get-simulation-execution-to-process';

export async function processSimulationExecution(simulationExecutionId: string) {
	const db = getDatabase();

	try {
		const executionConfig = await getSimulationExecutionToProcess(simulationExecutionId);
		...
		while (tradesCount < executionConfig.tradesToExecute) {
			...
			const vrpList = await getSimulationExecutionFrvpProfiles(executionConfig, tradeTime);
			...
			const aiResponse = await getDeepSeekResponse({
				prompt,
				system,
				apiKey: getEnv().DEEPSEEK_API_KEY_ID,
			...
			await db.insert(simulationExecutionLog).values({
				...
				direction:
					aiResponse.type === 'buy' ? TradeLogDirection.Buy : aiResponse.type === 'sell' ? TradeLogDirection.Sell : TradeLogDirection.Hold,
